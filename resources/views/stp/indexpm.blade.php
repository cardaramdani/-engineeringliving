...
@endsection
@section('li_tambahan')
<form action="{{url('pmstp')}}" method="post" class="form-inline col-6">
    @csrf
...
<!-- pagination dan titel -->
  @section('pagination-title')
    <div class="title">PM.<span>Sewage Treatment Plant</span></div>
  @endsection
...
@section('add')
    <li>
        <form id="ac-add-form" action="{{url('pmstp/add')}}" method="get" style="display: none;">
            @csrf
        </form>
  ...
  @section('body-tabel')
    @foreach ($pmSTP as $result => $pmstp)
      <tr>
        ...
        <td align="center">{{$pmstp->teknisi}}</td>
        <td align="center" >
        <ul class="ul-action">
            <form action="/pmstp/view/{{$pmstp->id}}" method="post" class="action-table " >
              @csrf
...
@role('Admin|Eng-spv|')
        <form action="/pmstp/edit/{{$pmstp->id}}" method="post" class="action-table" >
          @csrf
  ...
  @role('Admin|')
        <form action="/pmstp/delete/{{$pmstp->id}}" method="post" class="action-table"  onclick=" return confirm ('Yakin Mau Di Delet?');">
          @method('delete')
          @csrf
      ...
      <tr align="align-middle" class="th-footer">
        <td colspan="6" >
            <div class="footer-menu">
                <h6 >Showing {{$pmSTP->firstItem()}} - {{$pmSTP->lastItem()}} from {{$pmSTP->total()}}</h6>
                {{$pmSTP->onEachSide(1)->links()}}
            </div>
        </td>
      </tr>
  @endsection
